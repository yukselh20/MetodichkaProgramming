...
public class SpaceportDAO {
    ...
    public List<Spaceship> loadAllShips() {
        ...
        String sql = "SELECT id, name, tonnage, type, owner_id, x, y FROM spaceships";
    ...
    public boolean addShip(Spaceship ship) {
        String sql = "INSERT INTO spaceships (id, name, tonnage, type, owner_id, x, y) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DatabaseManager.getConnection();
            ...
            pstmt.setDouble(6, ship.getX());
            pstmt.setDouble(7, ship.getY());
            pstmt.executeUpdate();
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
