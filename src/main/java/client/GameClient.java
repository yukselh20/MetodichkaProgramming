...
public class GameClient {
  ...
  private void initiateMultiplayerConnection() {
    ui.displayMessage("Preparing for multiplayer...");
    // The changeState(ClientState.CONNECTING) call is removed because the next step will do it.

    if (!networkManager.connect()) {
      // Connection failed immediately
      changeState(ClientState.AUTH_MENU); // Go back to auth menu on failure
    } else {
      ui.displayMessage("Connection successful. Declaring local cases to server.");
      ...
      // This command is redundant if we send the case list, the server can just send back available
      // cases.
      // We'll keep it for now for consistency with the existing logic.
  ...
  private void handleQuitTypedCommand() {
    ClientState state = stateManager.getCurrentState();
    ui.displayMessage("'Quit' command typed in state: " + state);
