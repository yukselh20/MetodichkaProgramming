...
public class GameServer {
   ...
   */
  public void start() {
    System.out.println("Server event loop started.");
    try {
      while (running) {
        ...
        while (keyIterator.hasNext()) {
          ...
          try {
            ...
            System.err.println(
                "Error processing key for " + getClientInfo(key) + ": " + e.getMessage());
            e.printStackTrace();
            disconnectClient(key);
          }
        }
      ...
      System.out.println("Selector closed, server loop stopping.");
    } catch (IOException e) {
      System.err.println("IOException in server loop: " + e.getMessage());
    } finally {
      System.out.println("Server loop finished.");
      if (running) shutdown();
    }
  }
  ...
  private void handleAccept(SelectionKey key) throws IOException {
    System.out.println("[SERVER-DEBUG] ==> handleAccept() method has been entered.");
    ServerSocketChannel serverChannel = (ServerSocketChannel) key.channel();
    SocketChannel clientChannel = serverChannel.accept();
    if (clientChannel == null) {
      // --- NEW DEBUG LINE ---
      System.out.println("[SERVER-DEBUG] clientChannel was null, exiting handleAccept.");
      return;
    }

    // --- NEW DEBUG LINE ---
    System.out.println("[SERVER-DEBUG] Connection accepted from: " + clientChannel.getRemoteAddress());
    ...
    try {
      ...
      System.out.println("Client connected: " + getClientInfo(clientChannel) + ", ID: " + playerId);
      gameSessionManager.handleNewClientConnection(session);
    } catch (ClosedChannelException e) {
      System.err.println("Channel closed for new client " + playerId + " during registration.");
